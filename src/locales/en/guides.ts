// English translations - Guides section
export default {
  guides: {
    // Page title and hero section
    title: 'Security Knowledge Base',
    subtitle: 'In-depth guides on cryptography, payment security, and HSM management.',
    heroTagline: 'Master the concepts behind the tools.',
    searchPlaceholder: 'Search guides, algorithms, or concepts...',
    
    // Header button
    exploreTools: 'Explore Tools',
    
    // Category filters
    categories: {
      all: 'All Guides',
      concept: 'Concept',
      tutorial: 'Tutorial',
      bestPractice: 'Best Practice',
    },
    
    // Article categories (mapped from tool categories)
    articleCategories: {
      Keys: 'Key Management',
      Payment: 'Payment Security',
      Cipher: 'Encryption',
      Generic: 'General',
      PKI: 'PKI & Certificates',
    },
    
    // List page
    featuredGuides: 'Featured Guides',
    latestGuides: 'Latest Guides',
    allGuides: 'All Guides',
    readMore: 'Read more',
    minRead: 'min read',
    lastUpdated: 'Last Updated',
    noArticles: 'No articles found.',
    noSearchResults: 'No guides found for "{term}". Try searching for \'AES\' or \'Key\'.',
    
    // Article page
    onThisPage: 'On this page',
    relatedTool: 'Related Tool',
    openTool: 'Open Tool',
    needToCalculate: 'Need to calculate this now?',
    useOurTool: 'Use our free online {toolName} tool.',
    backToGuides: 'Back to Guides',
    readNext: 'Read Next',
    articleNotFound: 'Article Not Found',
    articleNotFoundDesc: 'The requested article could not be found.',
    invalidArticle: 'Invalid article',
    byTeam: 'By HSM Kit Team',
    translationNotice: 'This article is displayed in English. Translation is coming soon.',
    searchResults: 'Search Results',
    articlesCount: 'articles',
    
    // Breadcrumb
    home: 'Home',
    
    // SEO
    seoTitle: 'Security Knowledge Base - HSM Kit Guides',
    seoDescription: 'In-depth guides and tutorials on cryptography, payment security, key management, and HSM operations. Learn encryption concepts with practical examples.',
    seoKeywords: 'cryptography guide, payment security tutorial, HSM documentation, key management, TR-31, DUKPT, PIN block, KCV, encryption tutorial',
    
    // Tags
    tags: {
      KCV: 'KCV',
      XOR: 'XOR',
      Compliance: 'Compliance',
      'PCI DSS': 'PCI DSS',
      'TR-31': 'TR-31',
      'Key Block': 'Key Block',
      'ANSI X9.143': 'ANSI X9.143',
      HSM: 'HSM',
      'PIN Block': 'PIN Block',
      'ISO 9564': 'ISO 9564',
      'Payment Security': 'Payment Security',
      DUKPT: 'DUKPT',
      'Key Derivation': 'Key Derivation',
      POS: 'POS',
      ATM: 'ATM',
      CVV: 'CVV',
      CVC: 'CVC',
      'Card Security': 'Card Security',
      EMV: 'EMV',
    },
  },
};
