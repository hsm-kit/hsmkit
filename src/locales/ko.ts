...
export default {
  ...
  home: {
    ...
    features: {
      ...
      free: {
        title: '무료 & 오픈',
        description: '44개 이상의 도구가 모두 무료입니다. 등록 불필요, 로그인 불필요, 숨겨진 비용 없음. 즉시 사용 가능.',
      },
      paymentReady: {
        title: 'HSM & 결제 대응',
  ...
  seo: {
    home: {
      ...
      faqs: [
        { question: 'HSM Kit은 무료인가요?', answer: '네, HSM Kit은 44개 이상의 암호화 도구를 100% 무료로 제공합니다. 등록, 로그인, 결제 불필요. 모든 기능을 즉시 사용할 수 있습니다.' },
      ...
      usage: [
        '• HSM Kit은 보안 전문가, 개발자, 암호화 및 결제 시스템 작업자를 위해 설계된 44개 이상의 암호화 도구 모음입니다.',
    ...
    ansiMac: {
      ...
      usage: [
        '• MAC 알고리즘 선택: X9.9(도매)는 DES CBC-MAC, X9.19(소매)는 3DES 최종 블록.',
        '• 기본 DES 키 K(16개 16진수) 입력. X9.19에서 별도 키 사용 시 K\'(16개 16진수)도 입력.',
        '• 데이터를 16진수 형식으로 입력(8바이트 블록으로 자동 패딩).',
        '• 절단 길이 설정(1-8바이트). 대부분의 앱에서 표준은 4바이트.',
        '• "Calculate MAC" 클릭하고 결과 복사.'
      ],
    },
    as2805Mac: {
      ...
      usage: [
        '• MAC 방법 선택: 메소드 1(DES CBC) 또는 메소드 2(DES CBC + 3DES 최종 블록).',
        '• K|KL 키(16개 16진수)와 KR 키(16개 16진수) 입력.',
        '• 거래 데이터를 16진수 형식으로 입력.',
        '• 절단 길이 설정(1-8바이트, 표준 EFTPOS는 4바이트).',
        '• "MAC 계산" 클릭하고 결제 메시지용 결과 복사.',
      ],
    },
    iso9797Mac: {
    ...
    tdesCbcMac: {
      ...
      usage: [
        '• 키 유형 선택: 3키 TDES(48개 16진수, 최대 보안) 또는 2키 TDES(32개 16진수, 레거시 시스템).',
        '• TDES 키를 16진수 형식으로 입력.',
        '• 패딩 방법 선택: 방법 1(제로 패딩) 또는 방법 2(0x80 + 제로, 권장).',
        '• 메시지 데이터를 16진수 형식으로 입력.',
        '• 절단 길이를 설정(1-8바이트)하고 "MAC 계산" 클릭.',
      ],
    },
    hmac: {
      ...
      usage: [
        '• 해시 알고리즘 선택(대부분의 애플리케이션에 SHA-256 권장, 높은 보안에는 SHA-512).',
        '• 키 형식(16진수 또는 ASCII)을 선택하고 비밀 키 입력.',
        '• 데이터 형식(16진수 또는 ASCII)을 선택하고 인증할 메시지 입력.',
        '• "HMAC 계산" 클릭하여 인증 코드 생성.',
        '• HMAC 결과를 API 헤더, Webhook 또는 검증 시스템에 복사.',
      ],
    },
    cmac: {
      ...
      usage: [
        '• 암호화 알고리즘 선택: AES(권장) 또는 TDES(레거시 시스템용).',
        '• 키 형식을 선택하고 비밀 키 입력(AES: 32/48/64개 16진수; TDES: 32/48개 16진수).',
        '• 데이터 형식을 선택하고 인증할 메시지 입력.',
        '• 선택적: "AES CMAC 96"을 체크하여 96비트 절단 출력(IPsec/802.11i에서 일반적).',
        '• "CMAC 계산" 클릭하고 결과 복사.',
      ],
    },
    retailMac: {
      ...
      usage: [
        '• 알고리즘 선택: 레거시 시스템에는 DES, 전체 강화 보안에는 3DES.',
        '• 파이널라이제이션 선택: 표준 처리는 "없음", 최종 블록 보안 강화는 "3DES".',
        '• 키 K(16개 16진수) 입력. 3DES 파이널라이제이션 또는 3키 모드 사용 시 키 K\'도 입력.',
        '• 메시지 데이터를 16진수로 입력. ISO 9797-1 방법 2 패딩이 자동 적용.',
        '• 절단 길이(1-8바이트)를 설정하고 "MAC 계산" 클릭.',
      ],
    },
    zka: {
